...
async function checkAuth() {
  try {
    ...
    isAdmin = Clerk.user.primaryEmailAddress?.emailAddress === 'ksucetsac2024@gmail.com';
...
// Modify initHostelsPage to check auth first
async function initHostelsPage() {
  if (!await checkAuth()) return;
  
  // Rest of the existing initHostelsPage code...
}

// Modify initDetailPage to check auth first
async function initDetailPage() {
  if (!await checkAuth()) return;
  
  // Rest of the existing initDetailPage code...
}

// Modify initIndexPage to check auth first
async function initIndexPage() {
  if (!await checkAuth()) return;
  
  // Rest of the existing initIndexPage code...
}
